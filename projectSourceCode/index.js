...
// Leaderboard route with proper error handling and data formatting
app.get('/leaderboard', async (req, res) => {
  try {
    // Fetch stocks with their price change data
    const stocks = await db.any(`
      SELECT 
        s.Picker AS company,
        s.Price AS current_price,
        COALESCE(s.Previous_Price, s.Price * 0.95) AS previous_price,
        COALESCE(
          ROUND((s.Price - COALESCE(s.Previous_Price, s.Price * 0.95)) / 
          COALESCE(s.Previous_Price, s.Price * 0.95) * 100, 
          2
    ...
    const leaderboardData = {
      stocks: stocks.map((stock, index) => ({
        rank: index + 1,
        company: stock.company || 'Unknown Company',
        change: Number(stock.change_percentage) || 0,
        current_price: Number(stock.current_price).toFixed(2),
        previous_price: Number(stock.previous_price).toFixed(2)
      }))
    };

    res.render('pages/leaderboard', leaderboardData);
    ...
    res.render('pages/leaderboard', {
      stocks: [
        {rank: 1, company: "Apple", change: 2.5, current_price: "150.25", previous_price: "146.50"},
        {rank: 2, company: "Microsoft", change: -1.2, current_price: "310.75", previous_price: "314.50"},
        {rank: 3, company: "Google", change: 4.7, current_price: "2750.30", previous_price: "2625.50"}
      ],
      error: "Using sample data due to loading error"
    });
  }
});
